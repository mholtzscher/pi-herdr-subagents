...
const task = {
  ...
  id: "u1",
  parentId: null,
  message: { role: "user", content: "<!-- pi-herdr-task:task-1 -->\nTask" },
};
...
test("reads the final assistant descendant and truncates it", async () => {
  const path = await session([
    ...
    { type: "message", id: "t1", parentId: "a1", message: { role: "toolResult", content: [] } },
    {
      type: "message",
      id: "a2",
      parentId: "t1",
...
test("rejects manual interleaving and malformed ancestry", async () => {
  ...
  const interleaved = await session([
    header,
    task,
    { type: "message", id: "u2", parentId: "u1", message: { role: "user", content: "manual" } },
    { type: "message", id: "a1", parentId: "u2", message: { role: "assistant", content: "answer" } },
  ]);
  await assert.rejects(
    () => reader.read({ sessionPath: interleaved, taskId: taskIdFor(0), maxChars: 20_000 }),
    ResultAttributionError,
  );
  const badParent = await session([
    header,
    task,
    { type: "message", id: "a1", parentId: "missing", message: { role: "assistant", content: "answer" } },
  ]);
  await assert.rejects(
    () => reader.read({ sessionPath: badParent, taskId: taskIdFor(0), maxChars: 20_000 }),
    ResultAttributionError,
  );
});
