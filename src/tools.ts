...
export function registerSpawnPiTool(pi: ExtensionAPI, runner: BatchRunner, getParentLabel: () => string, configResult: ChildRolesConfigLoadResult): void {
  pi.registerTool({
    name: "spawn_pi",
    label: "Spawn Pi",
    ...
    async execute(_toolCallId, params, signal, onUpdate, ctx) {
      if (!configResult.ok) throw new Error(`Invalid Child Roles config at ${configResult.path}: ${configResult.error}`);
    ...
    renderCall(args, theme) {
      ...
      return new Text(theme.fg("toolTitle", theme.bold("spawn_pi ")) + theme.fg("accent", `${args.tasks.length} task${args.tasks.length === 1 ? "" : "s"}`) + (roles.length ? theme.fg("muted", `  ${roles.join(", ")}`) : ""), 0, 0);
    },
    renderResult(result, { expanded }, theme) {
      ...
      const lines = details.results.map((child) => {
        ...
        if (child.selection?.model) line += theme.fg("muted", `  ${child.selection.model.provider}/${child.selection.model.id}`);
        if (child.selection?.thinkingLevel) line += theme.fg("muted", `  thinking:${child.selection.thinkingLevel}`);
        if (expanded && (child.selection?.modelSource || child.selection?.thinkingSource)) line += theme.fg("muted", ` (${[child.selection.modelSource && `model:${child.selection.modelSource}`, child.selection.thinkingSource && `thinking:${child.selection.thinkingSource}`].filter(Boolean).join(", ")})`);
